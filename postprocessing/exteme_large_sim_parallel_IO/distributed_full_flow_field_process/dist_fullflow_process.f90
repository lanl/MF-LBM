...
MODULE misc
    ...
    real(kind=4), ALLOCATABLE, DIMENSION(:, :, :) :: phisp, usp, vsp, wsp, rhosp     !single precision version
...
program main_multiphase
    ...
    if (input_precision_choice == 1) then
        deallocate (phisp, usp, vsp, wsp, rhosp)
    end if
    ...
    do n_step = 1, ncount
        ...
        do id = 0, np - 1
            ...
            if (input_precision_choice == 1) then
                ...
                read (9 + id) (((phisp(i + idx*nx, j + idy*ny, k + idz*nz), i=1, nx), j=1, ny), k=1, nz)
            else
                read (9 + id) idx, idy, idz, nx, ny, nz
        ...
        if (save_vtk_choice == 1) then
            ...
            if (skip_point_choice == 1) then
                call VTK_detail_bin_half_sp(nt)  !single precision only for skip point output
            else
...
subroutine save_macro(nt)
    ...
    if (input_precision_choice == 1) then
      ...
      open (unit=9, file=trim(data_location)//'/processed_field_data/phi_sp_'//trim(flnm), FORM='unformatted', access='stream', status='replace', convert='BIG_ENDIAN')
        write (9) phisp
        close (9)
open (unit=9, file=trim(data_location)//'/processed_field_data/rho_sp_'//trim(flnm), FORM='unformatted', access='stream', status='replace', convert='BIG_ENDIAN')
        write (9) rhosp
        close (9)
open (unit=9, file=trim(data_location)//'/processed_field_data/vel_sp_'//trim(flnm), FORM='unformatted', access='stream', status='replace', convert='BIG_ENDIAN')
        write (9) usp
        write (9) vsp
        write (9) wsp
        close (9)
        ...
        write (flnm, '(i10.10,".dat")') nt
      open (unit=9, file=trim(data_location)//'/processed_field_data/phi_dp_'//trim(flnm), FORM='unformatted', access='stream', status='replace', convert='BIG_ENDIAN')
        write (9) phi
        close (9)
open (unit=9, file=trim(data_location)//'/processed_field_data/rho_dp_'//trim(flnm), FORM='unformatted', access='stream', status='replace', convert='BIG_ENDIAN')
        write (9) rho
        close (9)
open (unit=9, file=trim(data_location)//'/processed_field_data/vel_dp_'//trim(flnm), FORM='unformatted', access='stream', status='replace', convert='BIG_ENDIAN')
        write (9) u
        write (9) v
        write (9) w
        close (9)
...
subroutine VTK_detail_bin(nt)
    ...
    if (output_vtk_precision_choice == 1 .or. input_precision_choice == 1) then   ! dp in sp out / sp in sp out
        ...
        !scalar - phase field
        buffer = 'SCALARS phi '//fmt//lf; write (ivtk) trim(buffer)
        buffer = 'LOOKUP_TABLE default'//lf; write (ivtk) trim(buffer)
        write (ivtk) (((real(phi(i, j, k)), i=1, nxglobal), j=1, nyglobal), k=1, nzglobal)
