...
subroutine alter_color_gradient_solid_surface
  ...
  real(kind=8) :: cnx0, cny0, cnz0, cnxp, cnyp, cnzp, cnxm, cnym, cnzm, coe1, coe2, tmp1, tmp2, tmpCos, distP, distM

  local_eps = 1d-6

  !$omp parallel do default(none)&
  !$omp & private(i,j,k,nwx,nwy,nwz,cnx0, cny0, cnz0, cnxp, cnyp, cnzp, cnxm, cnym, cnzm, coe1, coe2, tmp1,tmp2,tmpCos, distP, distM, theta_local) &
  ...
  do num = 1, num_fluid_boundary
    ...
    if (c_norm(i, j, k) > local_eps) then
      ...
      cny0 = cn_y(i, j, k)
      cnz0 = cn_z(i, j, k)
      tmpCos = dcos(theta_local)
      tmp1 = nwx*cnx0 + nwy*cny0 + nwz*cnz0
      tmp2 = 1.0/dsqrt(1 - tmp1*tmp1)
      coe1 = dsin(theta_local)*tmp1*tmp2
      coe2 = dsin(theta_local)*tmp2
      cnxp = (tmpCos - coe1)*nwx + coe2*cnx0
      cnyp = (tmpCos - coe1)*nwy + coe2*cny0
      cnzp = (tmpCos - coe1)*nwz + coe2*cnz0
      cnxm = (tmpCos + coe1)*nwx - coe2*cnx0
      cnym = (tmpCos + coe1)*nwy - coe2*cny0
      cnzm = (tmpCos + coe1)*nwz - coe2*cnz0
